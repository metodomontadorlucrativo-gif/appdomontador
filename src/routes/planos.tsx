...
function TrialProgressCard({ sub }: { sub: import("@/lib/billing.functions").SubscriptionView }) {
  const totalDays = 30;
  ...
  return (
    <div className="mx-auto mb-6 max-w-2xl rounded-xl border border-brand/30 bg-brand/10 p-4 text-brand-dark">
      ...
      <div className="mt-4">
        <div className="h-2.5 w-full overflow-hidden rounded-full bg-brand/20">
          <div
            className="h-full rounded-full bg-brand transition-all duration-500"
            style={{ width: `${pct}%` }}
          />
        </div>
        <div className="mt-2 flex items-center justify-between text-xs font-semibold opacity-80">
          <span>
            {usedDays} dia{usedDays === 1 ? "" : "s"} usado{usedDays === 1 ? "" : "s"} de {totalDays}
          </span>
          <span>
